//! End-to-end integration tests for `FfiBackend`.
//!
...
//! cargo test --features ffi -p qwen-tts-runtime --test e2e -- --nocapture
//! ```
...
use qwen_tts_runtime::{DeviceKind, FfiBackend, RuntimeBackend, SynthesisRequest};
...
fn out_path(label: &str) -> PathBuf {
    let dir = env::temp_dir().join("qwen-tts-e2e-ffi");
...
#[test]
fn e2e_synthesis_with_instruct() {
    let Some(models) = get_models() else {
        eprintln!("SKIP: set QWEN_TTS_TALKER and QWEN_TTS_CODEC");
        return;
    };
    ...
    let path = out_path("instruct");
    let mut req = make_request("Hello world.", path, &models);
    req.instruct = Some("Speak in a cheerful, excited tone.".to_owned());
    req.speaker = Some("default".to_owned());

    let response = backend.synthesize(&req).expect("instruct synthesis");
    assert!(
        response.data_size_bytes > 200,
        "instruct output should be non-trivial"
    );
...
fn e2e_deterministic_seed() {
    ...
    // Run twice with the same seed — output WAVs should be byte-identical
...
#[test]
fn e2e_different_seed_produces_different_output() {
    let Some(models) = get_models() else {
        eprintln!("SKIP: set QWEN_TTS_TALKER and QWEN_TTS_CODEC");
        return;
    };
    ...
    let text = "Different seeds should give different audio output.";

    let path_a = out_path("diff_seed_a");
    let mut req_a = make_request(text, path_a, &models);
    req_a.seed = Some(1);
    req_a.do_sample = Some(true);

    let path_b = out_path("diff_seed_b");
    let mut req_b = make_request(text, path_b, &models);
    req_b.seed = Some(99999);
    req_b.do_sample = Some(true);

    let resp_a = backend.synthesize(&req_a).expect("seed=1");
    let resp_b = backend.synthesize(&req_b).expect("seed=99999");

    let wav_a = std::fs::read(&resp_a.wav_path).expect("read A");
    let wav_b = std::fs::read(&resp_b.wav_path).expect("read B");

    assert_ne!(
        wav_a, wav_b,
        "different seeds should produce different WAV output"
    );

    eprintln!("PASS: different seeds produce different output (A={}, B={})", wav_a.len(), wav_b.len());
}
